...
Gateway will only log successful override connections in your [network logs](/cloudflare-one/insights/logs/gateway-logs/#network-logs). If the override destination IP does not exist, Gateway will override the destination IP but not log the override action.
